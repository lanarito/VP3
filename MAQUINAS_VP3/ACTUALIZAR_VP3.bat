...
echo.
echo [1/12] Cerrando procesos viejos...
echo [%date% %time%] llegue a 1/12 >> "%TEMP%\vp3_debug.log"
...
echo [2/12] Descargando ultima version desde GitHub...
echo [%date% %time%] llegue a 2/12 >> "%TEMP%\vp3_debug.log"
...
echo [3/12] Extrayendo archivos...
echo [%date% %time%] llegue a 3/12 >> "%TEMP%\vp3_debug.log"
...
echo [4/12] Copiando archivos nuevos...
echo [%date% %time%] llegue a 4/12 >> "%TEMP%\vp3_debug.log"
...
echo [5/12] Limpiando archivos temporales...
del "%TEMP%\MAQUINAS_VP3_NUEVO.zip" >nul 2>&1
rmdir /S /Q "%TEMP%\VP3_TEMP" >nul 2>&1
echo       OK
echo.

echo [%date% %time%] llegue a 5/12 >> "%TEMP%\vp3_debug.log"
echo [6/12] Aplicando fix de error al apagar (registro Windows)...
...
echo [%date% %time%] llegue a 6/12 >> "%TEMP%\vp3_debug.log"
echo [7/12] Configurando Windows Error Reporting y exclusiones del antivirus...
...
echo [%date% %time%] llegue a 7/12 >> "%TEMP%\vp3_debug.log"
echo [8/12] Registrando sincronizacion final antes de apagar...
powershell -NoProfile -ExecutionPolicy Bypass -File "%VP3_DESTINO%registrar_sync_apagado.ps1" >nul 2>&1
echo       OK
echo.

echo [%date% %time%] llegue a 8/12 >> "%TEMP%\vp3_debug.log"
...
echo [9/12] Arreglando carteles de error conocidos en mesas...
...
REM ENCONTRADO 16-sep-2026: al agregar 59 mesas nuevas se colaron en la
REM nube 43 records que no son de ningun jugador (tablas de fabrica de
REM las mesas nuevas, mas algunos con las iniciales mal grabadas). El
REM filtro que los dejaba pasar ya esta arreglado en subir_puntajes.exe,
REM pero los que YA habian subido hay que sacarlos de la nube.
REM Corre aca, DESPUES de copiar los archivos nuevos y ANTES de arrancar
REM el watchdog: asi el programa que los subia esta parado mientras se
REM limpia, y cuando arranca ya tiene el filtro nuevo puesto.
REM La lista de los 43 esta escrita adentro del programa, revisada uno
REM por uno -- no decide nada solo. Si ya estan borrados (porque otra
REM maquina actualizo antes), no hace nada. Guarda respaldo con fecha.
echo [11/12] Limpiando jugadores fantasma de la nube...
"%VP3_DESTINO%LIMPIAR_FANTASMAS.exe" --auto
echo       OK
echo.

echo [%date% %time%] llegue a 11/12 >> "%TEMP%\vp3_debug.log"
echo [12/12] Iniciando watchdog actualizado...
...
echo ===============================================
echo [%date% %time%] llegue a 12/12, LISTO >> "%TEMP%\vp3_debug.log"
